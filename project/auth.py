...
def signup_post():
    ...
    query = text('select * from user where email =:email') #PARAMETERISATION AGAINST INJECTION
    user = db.session.execute(query, {"email": email}).all()

    # OLD user = db.session.execute(text('select * from user where email = "' + email +'"')).all()
